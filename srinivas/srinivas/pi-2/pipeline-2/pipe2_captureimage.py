...
def comppute_compress_QoE():
    ...
    client.connect('10.5.0.156', username='pi', password='lockthis')
    ...
    #print('The Round Trip Time in sec on 10.5.0.156 (pi-1) when running compression code is {} '.format(elapsed_time_pi1))
    ...
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect('10.5.1.32', username='pi', password='lockthis')
    ...
    f = open("/home/pi/pipeline-2/ic_output_of_jar.txt")
    k = float(f.readline())
    Ccs.append(k)
    #print("CCs of Pi-1",k)
    # print("RoE", RoE)
    ...
    print("Selected Node for performing image compression operation::::", max_QoE_index)
    ...
    if(max_QoE_index == 1):
        ...
        sftp = client.open_sftp()
        try:
            f = sftp.open("/home/pi/Compression_token.txt", 'w')
        except IOError:
            pass
        f.write("compress")
        f.close()

...
import array as arr
import operator
from executejarfrompy import computeRoE, computeCcs, computeQoE
...
client.connect('10.5.0.54', username='administrator', password='zen')
...
print('The Round Trip Time in sec on 10.5.0.209 (pi-1) when running compression code is {} '.format(elapsed_time_pi1))
...
client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect('10.5.0.190', username='charan', password='root123')
...
print('The Round Trip Time in sec on 10.5.1.32 (pi-2) when running compression code is {} '.format(reslut2))
...
stdin, stdout, stderr = client.exec_command('python3 /home/administrator/Contribution-3/Automate_pipeline/getcpuspeed.py')
...
stdin, stdout, stderr = client.exec_command('python3 /home/charan/srinivas/getcpuspeed.py')
...
stdin, stdout, stderr = client.exec_command('python3 /home/administrator/Contribution-3/Automate_pipeline/getavailableresources.py')
...
stdin, stdout, stderr = client.exec_command('python3 /home/charan/srinivas/getavailableresources.py')
...
Ccs = []
computeCcs(Nor_Round_Trip_pi1, Nor_A_r_pi1, Nor_p_s_pi1)
...
computeCcs(Nor_Round_Trip_pi2, Nor_A_r_pi2, Nor_p_s_pi2)
f = open("output_of_jar.txt")
k = float(f.readline())
Ccs.append(k)
print("Ccs of Pi-2",k)
...
computeRoE(Nor_image_compression_accessrate, Nor_image_compression_requiredresources, Nor_image_compression_processingtime)
f = open("output_of_jar.txt")
k = float(f.readline())
RoE.append(k)
...
if(max_QoE_index == 0):
    ...
    try:
        f = sftp.open("/home/administrator/Contribution-3/connect-to-mqtt/Compression_token.txt", 'w')
    except IOError:
        pass
    f.write("extract")
    f.close()
...
if(max_QoE_index == 1):
    ...
    sftp = client.open_sftp()
    try:
        f = sftp.open("/home/charan/srinivas/Compression_token.txt", 'w')
    except IOError:
        pass
    f.write("extract")
    f.close()

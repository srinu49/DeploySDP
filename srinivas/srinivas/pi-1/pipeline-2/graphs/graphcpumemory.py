...
def plot_cpu_memory_graph(interval_sec):
    ...
    # Plot memory usage
    plt.subplot(2, 1, 2)
    plt.plot( memory_percentages, color='r')
    ...
    output_file_path =  '/home/pi-cv-101/pipeline-2/graphs/' 
    plt.savefig(output_file_path + str(current_datetime) +"node3 "+'.png')
    print(f"Graph saved to: {output_file_path}")

# Call the function with a 20-second interval
plot_cpu_memory_graph(2200)

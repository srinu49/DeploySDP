...
#def publish_to_broker(image_content):
    ...
    #client.loop_start() 
    #client.publish("/testmqtt/frompi", image_content,0)
    #print("published resized image on to the desktop cmsd and topic name is /testmqtt/frompi")
    #time.sleep(10)
    #client.loop_stop()
...
def on_message(client, userdata, message):
    ...
    # print(f)
    # f.close()
    print("Image Received at the topic")
    image_data = message.payload
    image = Image.open(io.BytesIO(image_data))
    image.save("/opt/human-detection/resized_image.png")
...
all_words = []
with open("/opt/human-detection/pi_ipadd.txt", "r") as file:
# Read each line
    for line in file:

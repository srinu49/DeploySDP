...
stdin, stdout, stderr = client.exec_command('python3 /opt/human-detection/getcpuspeed.py')
...
f = open("/opt/human-detection/ri_output_of_jar.txt")
k = float(f.readline())
Ccs.append(k)
print("CCs of Pi-1",k)
# print("RoE", RoE)
...
if(random_max_index == 0):
    ...
    try:
        f = sftp.open("/opt/human-detection/Resize_token.txt", 'w')
    except IOError:
        pass
    f.write("resize")
    f.close()
